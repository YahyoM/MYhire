        ...
        >
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1">
              ...
              <h3 className="mb-2 text-2xl font-bold text-slate-800 sm:text-3xl">
                {job.title}
              </h3>
            ...
            <button
              className="group flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-blue-200/50 text-slate-700 transition-all hover:border-red-400/50 hover:bg-red-500/10 hover:text-red-600"
              onClick={onClose}
            >
          ...
          <form className="relative mt-6 space-y-4" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2">
              <label className="flex flex-col gap-2">
                <span className="text-sm font-medium text-slate-700">
                  ...
                  {hasProfileData && (
                    <span className="ml-2 text-xs font-normal text-blue-600">(from your profile)</span>
                  )}
                </span>
                <input
                  className={`rounded-xl border border-blue-200/50 px-4 py-3 text-slate-800 placeholder:text-slate-500 transition-all focus:border-blue-400/60 focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${
            ...
            <label className="flex flex-col gap-2">              <span className="text-sm font-medium text-slate-700">
                Your Skills
                {profile?.skills && profile.skills.length > 0 && (
                  <span className="ml-2 text-xs font-normal text-blue-600">(from your profile)</span>
                )}
              </span>
              <div className="flex flex-wrap gap-2 rounded-xl border border-blue-200/50 bg-slate-50 px-4 py-3 text-slate-800">
                {skills.length > 0 ? (
            ...
            <label className="flex flex-col gap-2">              <span className="text-sm font-medium text-slate-700">Cover Letter</span>
              <textarea
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-3 rounded-xl border-2 border-dashed border-blue-400/40 bg-slate-50 p-4 transition-all focus-within:border-blue-400/70">
                {profile?.resumeUrl && !fileMeta ? (
            ...
            <div className="flex flex-col gap-3 border-t border-blue-200/50 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="flex items-center gap-2 text-xs text-slate-600">
                <svg className="h-4 w-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                Ваши данные будут надёжно переданы в {job.company}
              </p>
              <button
                type="submit"
                disabled={loading || isSubmitted}
                className={`group relative overflow-hidden rounded-full px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 ${
                  isSubmitted 
